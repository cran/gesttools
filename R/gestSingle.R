...
#' Cn=NA
#' LnC=NA
#' gestSingle(data,idvar,timevar,Yn,An,Ybin,Abin,Acat,Lny,Lnp,type=1)
#'
...
#' Cn="C"
#' LnC=c("L","U")
#' gestSingle(data,idvar,timevar,Yn,An,Ybin,Abin,Acat=FALSE,Lny,Lnp,type=1,Cn,LnC)
#'
...
gestSingle<-function(data,idvar,timevar,Yn,An,Ybin,Abin,Acat,Lny,Lnp,type,Cn=NA,LnC=NA,...){
  # Error messages
  ...
  if(Acat==TRUE){d[,Alagn]<-as.factor(d[,Alagn])
  levels(d[,paste("L",1,An,sep="")])<-levels(d[,An])
  d[d[,timevar] %in% 1,Alagn]<-levels(d[,An])[1]
  ...
  } else {modp<-glm(lmp,family="gaussian",data=d)}
  ...
  d$H<-0
  d[d[,timevar]==T,"H"]<-d[d[,timevar]==T,Yn]
    ...
    if (Acat==TRUE){
      ...
      while(i>=1){
        ...
        while(j>=(i+1)){
          if(length(z)==1){
            for (l in 1:nlevels(d[,An])){
              d[d[,timevar]==j & d[,An]==levels(d[,An])[l] & !is.na(d[,An]),"psiZA"]<-psicat[[l]]}
          }else{
            for (l in 1:nlevels(d[,An])){
              d[d[,timevar]==j & d[,An]==levels(d[,An])[l] & !is.na(d[,An]),"psiZA"]<-rowSums(
                sweep(d[d[,timevar]==j & d[,An]==levels(d[,An])[l] & !is.na(d[,An]),z],2,psicat[[l]],"*"))}
          }
          j<-j-1
        }
        ...
        while(k>=i){
          if(is.na(Cn)==FALSE){
            # Censoring weights
            d[d[,timevar]==k,"cprod"]<-d[d[,timevar]==k+1,"cprod"]*d[d[,timevar]==k,"cps"]
            d[d[,timevar]==k,"w"]<-d[d[,timevar]==T,paste(Cn,"0",sep="")]/d[d[,timevar]==k,"cprod"]
          }
          # Counterfactuals
          if (Ybin==FALSE){
            d[d[,timevar]==k,"H"]<-d[d[,timevar]==(k+1),"H"]-
              d[d[,timevar]==(k+1),"psiZA"]
          }
          if (Ybin==TRUE){
            d[d[,timevar]==k,"H"]<-d[d[,timevar]==(k+1),"H"]*
              exp(-d[d[,timevar]==(k+1),"psiZA"])
          }
        ...
        dt<-d[d[,timevar] %in% seq(i,T,by=1),]
    ...
    while(i>=1){
      ...
      while(k>=i){
        ...
        if(Ybin==FALSE){
          if (length(z)==1){
            d[d[,timevar]==k,"H"]<-d[d[,timevar]==(k+1),"H"]-
              d[d[,timevar]==(k+1),An]*psi*d[d[,timevar]==(k+1),z]
          }else{
            d[d[,timevar]==k,"H"]<-d[d[,timevar]==(k+1),"H"]-
              rowSums(d[d[,timevar]==(k+1),An]*sweep(d[d[,timevar]==(k+1),z],2,psi,"*"))
          }
        }
        if(Ybin==TRUE){
          if (length(z)==1){
            d[d[,timevar]==k,"H"]<-d[d[,timevar]==(k+1),"H"]*
              exp(-d[d[,timevar]==(k+1),An]*psi*d[d[,timevar]==(k+1),z])
          }else{
            d[d[,timevar]==k,"H"]<-d[d[,timevar]==(k+1),"H"]*
              exp(-rowSums(d[d[,timevar]==(k+1),An]*sweep(d[d[,timevar]==(k+1),z],2,psi,"*")))
          }
        }
        # Current time period is lowered and censoring weights and counterfactuals
        # calculated until current value of i is reached.
  ...
  if(timevarying==TRUE){
    ...
    while(i>=1){
      ...
      while(j>=(i+1)){
        if(length(z)==1){
          for (l in 1:nlevels(d[,An])){
            d[d[,timevar]==j & d[,An]==levels(d[,An])[l] & !is.na(d[,An]),"psiZA"]<-psicatlist[[j]][[l]]}
        }else{
          for (l in 1:nlevels(d[,An])){
            d[d[,timevar]==j & d[,An]==levels(d[,An])[l] & !is.na(d[,An]),"psiZA"]<-rowSums(
              sweep(d[d[,timevar]==j & d[,An]==levels(d[,An])[l] & !is.na(d[,An]),z],2,psicatlist[[j]][[l]],"*"))}
        }
        j<-j-1
      }
      # Obtain counterfactual outcomes and censoring weights for time T-1
    ...
    while(i>=1){
      ...
      while(k>=i){
        ...
        if (Ybin==FALSE){
          if (length(z)==1){
            d[d[,timevar]==k,"H"]<-d[d[,timevar]==(k+1),"H"]-
              d[d[,timevar]==(k+1),An]*psilist[[k+1]]*d[d[,timevar]==(k+1),z]
          }else{
            d[d[,timevar]==k,"H"]<-d[d[,timevar]==(k+1),"H"]-
              rowSums(d[d[,timevar]==(k+1),An]*sweep(d[d[,timevar]==(k+1),z],2,psilist[[k+1]],"*"))
          }
        }
        if (Ybin==TRUE){
          if (length(z)==1){
            d[d[,timevar]==k,"H"]<-d[d[,timevar]==(k+1),"H"]*
              exp(-d[d[,timevar]==(k+1),An]*psilist[[k+1]]*d[d[,timevar]==(k+1),z])
          }else{
            d[d[,timevar]==k,"H"]<-d[d[,timevar]==(k+1),"H"]*
              exp(-rowSums(d[d[,timevar]==(k+1),An]*sweep(d[d[,timevar]==(k+1),z],2,psilist[[k+1]],"*")))
          }
        }
        k<-k-1
      }
